...
import { View, Plant, UserProfile, IdentifyResult, DiagnosisResult, Achievement, Reminder, ReminderType, ChatMessage, WeatherData } from './types';
...
import {
    ...
    Scissors, Shovel, Send, Bot, MessageCircle, Crown, Wifi
} from 'lucide-react';
        ...
        setPlants(prevPlants => prevPlants.map(p => {
            ...
            if (selectedPlant && selectedPlant.id === plantId) {
                const updatedReminders = p.reminders.map(r => {
                    ...
                    const nextDueDate = isWatering
                        ? calculateNextWatering(p, r.frequencyDays, {
                            temp: 25,
                            humidity: 60,
                            condition: 'Clear',
                            icon: '☀️',
                        ...
                        : new Date(Date.now() + (r.frequencyDays * 24 * 60 * 60 * 1000)).toISOString();
        ...
        <div className="space-y-8 pb-40 pt-4">
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm">
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex justify-between items-center p-3 bg-gray-50 rounded-xl opacity-60">
                        <div className="flex items-center gap-3">
                            <Wifi size={20} className="text-gray-500" />
                            <span className="text-gray-600">Soil Sensors</span>
                        </div>
                        <span className="bg-gray-200 text-gray-500 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wide">Coming Soon</span>
                    </div>
                </div>
            </div>
        </div>
